...
sns.swarmplot(
    ...
    size=14,
    alpha=0.90,
    edgecolor='black',          # ← changed to black (like first script)
    linewidth=1.0,
    dodge=False,
    legend=False,
    ax=ax_left
...
ax_left.set_ylabel("(S)     ee 3b (%)     (R)", fontsize=18, fontweight='bold', color='black')
ax_left.tick_params(axis='both', labelsize=18, colors='black')  # ← black ticks
...
if second_col:
    ...
    ax_right.set_ylabel("Product formation 3b (R) [mM]", fontsize=18, fontweight='bold', color='black')
    ax_right.tick_params(axis='both', labelsize=18, colors='black')  # ← black ticks
