...
from sklearn.metrics import roc_curve, roc_auc_score, matthews_corrcoef, confusion_matrix, ConfusionMatrixDisplay
import argparse
import os

parser = argparse.ArgumentParser(description="Evaluate solubility predictors with ROC curves + cutoff metrics")
parser.add_argument("-i", "--input", required=True, help="Path to the input CSV file (e.g., soluble_expression.csv)")
...
plt.plot([0, 1], [0, 1], color='gray', lw=1.0, linestyle='--', alpha=0.7, label='Random (AUC = 0.50)')
...
for auc, col, fpr, tpr, color in curves:
    plt.plot(fpr, tpr, lw=2, color=color, label=f'{col}  ({auc:.2f})')
...
for predictor in PREDICTOR_COLS:
    ...
    for cutoff in CUT_OFFS:
        ...
        ax.set_title(f"{predictor} \n Cutoff = {cutoff:.2f}", fontsize=16, fontweight='bold', pad=12)
        ...
        safe_name = predictor.replace(" ", "_").replace("(", "").replace(")", "").replace("/", "_")
        fname = f"cm_{safe_name}_cutoff_{cutoff:.2f}.svg"
