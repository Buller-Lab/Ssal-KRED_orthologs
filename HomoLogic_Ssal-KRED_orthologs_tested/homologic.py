...
class HomoLogic:
    ...
    def calculate_sequence_homology(self, output_folder='sequence_homology_results', substitution_matrix_name='BLOSUM62'):
        ...
        cmd = ['mmseqs', 'easy-search', '--max-seqs', '100000', self.reference_fasta, self.homologs_fasta, result_m8, tmp_dir]
        subprocess.run(cmd, check=True)

        homolog_seqs = {seq.metadata['id'].strip(): seq for seq in io.read(self.homologs_fasta, format='fasta', constructor=Protein)}
        ...
        self.print_progress(f"MMseqs2: {len(good_hits)} good | {len(fallback_targets)} fallback | {len(unmatched)} unmatched")
                ...
                if stats:
                    for i, r in enumerate(rows):
                        if r[1].strip() == target_id:
                            rows[i] = [
                                ref_id, target_id, stats['identity_pct_str'], str(stats['full_alignment_length']),
                                str(stats['mismatches']), str(stats['gap_openings']),
                                '1', str(len(ref_seq)), '1', str(len(tgt_seq)),
                                r[10], r[11]
                            ]
                            break
